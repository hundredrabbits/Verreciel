...
class StructureSatellite : Structure
{
	...
	override init()
	{
		...
		while i < nodes {
			...
			let shape = ShapeHexagon(size: 3, color: red)
			shape.position.x = 0
			axis.addChildNode(shape)
			
			let shape2 = ShapeHexagon(size: 3, color: red)
			shape2.eulerAngles.z = (degToRad(90))
			shape.addChildNode(shape2)
			
			let shape3 = ShapeHexagon(size: 3, color: red)
			shape3.eulerAngles.y = (degToRad(90))
			shape.addChildNode(shape3)
			
			let shape4 = ShapeHexagon(size: 3, color: red)
