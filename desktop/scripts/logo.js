...
function Logo(target_element, is_looping) {
  ...
  function animate_tile(tile, count) {
    ...
    if (parseInt(tile.element.style.left, 10) < tile.pos.x * tile.tile_size) {
      tile.translate_with(1, 0);
    }
    if (parseInt(tile.element.style.left, 10) > tile.pos.x * tile.tile_size) {
      tile.translate_with(-1, 0);
    }
    if (parseInt(tile.element.style.top, 10) < tile.pos.y * tile.tile_size) {
      tile.translate_with(0, 1);
    }
    if (parseInt(tile.element.style.top, 10) > tile.pos.y * tile.tile_size) {
      tile.translate_with(0, -1);
    }
    setTimeout(function() {
      animate_tile(tile, count - 1);
    }, 1);
  }
  ...
  function animate() {
    ...
    setTimeout(function() {
      animate_return_to(3, 99);
    }, 2000);
    setTimeout(function() {
      animate_return_to(2, 99);
    }, 2500);
    setTimeout(function() {
      animate_return_to(1, 99);
    }, 3000);
  ...
  function Tile(pos) {
    ...
    function tile_at(target_pos, neighboors) {
      for (t2 = 0; t2 < neighboors.length; t2++) {
        if (
          neighboors[t2].pos.x == target_pos.x &&
          neighboors[t2].pos.y == target_pos.y
        ) {
    ...
    this.translate_with = function(x, y) {
      this.element.style.left =
        parseInt(this.element.style.left, 10) + x + "px";
